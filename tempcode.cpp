...
//Semaphore variable for mutual exclusion
int sem = 1, sem1 = 1, sem2 = 1;
struct options
{
...
public:
  ...
  vector<string> addressPool; //A vector to store all thr Ip addresses available with the server
...
void dhcpCliAckSequence(clientNode *client, int recvServer)
{
  ...
  if (message1->opt->messageType == "DHCPACK" && message1->opt->serverId == recvServer)
  {
    ...
    cout << "Client " << client->clientId << " has been assigned the IP Address of ";
...
void *serverFunc(void *args)
{
  ...
  this_thread::sleep_for(chrono::microseconds(70 * (rand() % 10)));
  while (1)
  {
    ...
    if (server->recieveMessageBuffer.size() > 0)
    {
      ...
      // For dhcp request for renewal from the client
      if (req == "DHCPREQUEST")
      {
  ...
  while (1)
  {
    ...
    if (client->sendMessageBuffer.size() > 0)
    {
      ...
      if (sendMess->opt->messageType == "DHCPDISCOVER")
      {
        ...
        while (client->recieveMessageBuffer.size() != 3)
        ...
        if (message1->opt->clientId == client->clientId && message1->opt->messageType == "DHCPOFFER")
        {
          ...
          cout << "\n\nDHCPOFFER recieved from " << message1->opt->serverId;
          ...
          while (client->recieveMessageBuffer.size() > 0)
          {
            ...
            client->recieveMessageBuffer.pop_front();
            cout << "\nDHCP offer recieved from " << temp->opt->serverId << " is late and has been discarded\n";
          }

          cout << "\n\nServer " << recvServer << " has been chosen \nby the Client ";
          cout << client->clientId << " For recieving the address \n";
          client->recieveMessageBuffer.clear();
          this_thread::sleep_for(chrono::microseconds(20));

          //Check wheteher Ip is valid or not (ARP used in bg)
          // We are not concerned with that for DHC simulation
      ...
      if (sendMess->opt->messageType == "DHCPREQUEST")
      {
        ...
        //Ack message recieved back to the client
...
int main()
{
  //We are using these three servers for our task of simulation
  //These servers are of the given configuration(See the class serverNode for more details)
  ...
  //populating for client accordingly (information known to us before hand)
